...
class HangmanGame {
	...
	_wrongGuess() {
		if((--this.remaining_tries) === 0) {
			...
			this.sendMsg(`Źle! ${this.user_guess} Pozostało prób: ${this.remaining_tries}`);
	}
	...
	onGuess(guess) {
		...
		if(guess.replace(/[^a-z ąęśćółżźń]/gi, '') !== guess)
			return this.sendMsg('W haśle nie ma nic prócz liter i spacji. Próba zignorowana.');
		
		guess = guess.toUpperCase();
		
		if(this.guesses_history.has(guess))
			return this._repeatedGuess();
		...
		if(guess.length > 1) {
			...
			if(this.user_guess.indexOf(guess) !== -1)
				return this._repeatedGuess();//should never occur
			...
			if (guess_index === -1)
				...
				for(let i=0; i<this.target_entry.length; i++) {
					if(this.target_entry[i] === guess)
						arr[i] = guess;//replace dash with user's guessed letter
				}
...
module.exports = {
	...
	onPrivateMessage(invokerid, msg) {
		...
		if(game) {
			if( !msg.startsWith('!') )//just a guess
				game.onGuess(msg);
			else {
